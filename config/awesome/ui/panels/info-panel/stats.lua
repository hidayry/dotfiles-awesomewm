...
local function cpu()
	...
	watch(
		...
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(cpu_value .. "% CPU" , "#c1ebff"))
	...
	do
		...
		function(stdout)
			...
			sh -c "cat ]] .. temp_path .. [["
				...
				tooltip:set_markup_silently(helpers.ui.colorize_text(temp_value .. "°C Temp", "#a3cdff"))
				collectgarbage("collect")
			end)
		end
	)
...
local function ram()
	...
	watch(
		[[sh -c "
		free -m | grep workarea'Mem:' | awk '{printf \"%d@@%d@\", $7, $2}'
		"]],
		20,
		function(_, stdout)
			...
			tooltip:set_markup_silently(helpers.ui.colorize_text(string.format("%.1f", used / 1000) .. "G Ram", "#85afe7"))
			collectgarbage("collect")
		end
	)
...
local function hdd()
	...
	watch([[bash -c "df -h /home|grep '^/' | awk '{print $5}'"]], 180, function(_, stdout)
		...
		tooltip:set_markup_silently(helpers.ui.colorize_text(space_consumed .. "% HDD", "#6791c9"))
		collectgarbage("collect")
	end)
...
return create_boxed_widget(stats, dpi(270), dpi(220), beautiful.widget_bg)
